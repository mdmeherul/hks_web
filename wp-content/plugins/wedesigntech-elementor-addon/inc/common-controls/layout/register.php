...
class WeDesignTech_Common_Controls_Layout {
	...
	public function init_styles() {
		...
		if(\Elementor\Plugin::$instance->preview->is_preview_mode() || $this->settings['layout'] == 'carousel') {
			$layout_styles['wdt-swiper'] = WEDESIGNTECH_ELEMENTOR_ADDON_DIR_URL.'inc/common-controls/layout/assets/css/swiper.min.css';
		...
		$this->cc_style->get_style_controls($elementor_object, array (
			...
			'styles' => array (
				...
				'gap' => array (
					...
					'size_units' => array ( 'px' ),
					'selector' => array (
						'{{WRAPPER}} .wdt-carousel-holder .wdt-carousel-pagination-wrapper .wdt-swiper-pagination.swiper-pagination-bullets.swiper-pagination-horizontal .swiper-pagination-bullet' => 'margin: 0 {{SIZE}}{{UNIT}};'
					),
					'separator' => 'before',
					'condition' => array ()
				),
	...
	public function get_wrapper_start() {
		...
		if($layout == 'carousel') {
			...
			$wrapper_start .= '<div class="wdt-'.esc_attr($module_class).'-holder wdt-content-item-holder wdt-carousel-holder '.esc_attr($classes_str).'" id="wdt-'.esc_attr($module_class).'-'.esc_attr($module_id).'" data-id="'.esc_attr($module_id).'">';
	...
	public function get_carousel_attributes() {
		...
		foreach($breakpoint_keys as $breakpoint) {
			...
			array_push($swiper_breakpoints, array (
					'breakpoint' => $active_breakpoints[$breakpoint]->get_value()
				)
			);
