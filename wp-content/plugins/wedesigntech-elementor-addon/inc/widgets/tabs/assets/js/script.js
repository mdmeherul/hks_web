(function ($) {
  ...
  const wdtTabsWidgetHandlerInit = function($scope) {
    ...
    let $activeItem;
    let $classes = $scope.find('.wdt-tabs-container').data('class-items').split(' ');
    ...
    $self.init = function() {
      ...
      if($device === 'mobile' || $device === 'mobile_extra') {
        ...
        $.each($classes, function(key, value) {
          $scope.find('.wdt-tabs-container').removeClass(value);
        });
      ...
      $window.on('resize', () => {
        ...
        if($device === 'mobile' || $device === 'mobile_extra') {
          ...
          setTimeout(function () {
            ...
            $.each($classes, function(key, value) {
              $scope.find('.wdt-tabs-container').addClass(value);
            });
