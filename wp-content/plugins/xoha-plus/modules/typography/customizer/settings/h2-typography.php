...
if( !class_exists( 'XohaPlusH2Settings' ) ) {
    class XohaPlusH2Settings {
        ...
        function __construct() {
            $this->selector = apply_filters( 'xoha_h2_selector', array( 'h2' ) );
            $this->settings = xoha_customizer_settings('h2_typo');

            add_filter( 'xoha_plus_customizer_default', array( $this, 'default' ) );
            add_action( 'customize_register', array( $this, 'register' ), 20);

            add_filter( 'xoha_h2_typo_customizer_update', array( $this, 'h2_typo_customizer_update' ) );
        ...
        function register( $wp_customize ) {
            ...
            $wp_customize->add_section(
                new Xoha_Customize_Section(
                    ...
                    'site-h2-section',
                    array(
                        'title'    => esc_html__('H2 Typography', 'xoha-plus'),
             ...
             */
                $wp_customize->add_setting(
                    XOHA_CUSTOMISER_VAL . '[h2_typo]', array(
                        'type'    => 'option',
                    )
                );
                ...
                $wp_customize->add_control(
                    new Xoha_Customize_Control_Typography(
                        $wp_customize, XOHA_CUSTOMISER_VAL . '[h2_typo]', array(
                            'type'    => 'wdt-typography',
                            'section' => 'site-h2-section',
                            'label'   => esc_html__( 'H2 Tag', 'xoha-plus'),
                        )
                    )
                );
             ...
             */
                $wp_customize->add_setting(
                    XOHA_CUSTOMISER_VAL . '[h2_color]', array(
                        'default' => '',
                        'type'    => 'option',
                    )
                );
                ...
                $wp_customize->add_control(
                    new WP_Customize_Color_Control(
                        $wp_customize, XOHA_CUSTOMISER_VAL . '[h2_color]', array(
        ...
        function h2_typo_customizer_update( $defaults ) {
            $h2_typo = xoha_customizer_settings( 'h2_typo' );
            if( !empty( $h2_typo ) ) {
                return  $h2_typo;
            }
            return $defaults;
        ...
        function base_style( $style ) {
            $css   = '';
            $color = xoha_customizer_settings('h2_color');
