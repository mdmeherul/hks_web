...
class Xoha_Customize_Control_Typography extends WP_Customize_Control {
	...
	public function enqueue() {
		...
		wp_enqueue_script( 'xoha-plus-typography-control', XOHA_PLUS_DIR_URL.'customizer/controls/typography/typography.js', array( 'jquery', 'customize-base' ), XOHA_PLUS_VERSION, true );
		wp_enqueue_style( 'xoha-plus-typography-control',  XOHA_PLUS_DIR_URL.'customizer/controls/typography/typography.css', null, XOHA_PLUS_VERSION );

		$typo_localize = array(
			'inherit' => esc_html__( 'Inherit', 'xoha-plus' ),
			'100'     => esc_html__( 'Thin 100', 'xoha-plus' ),
			'200'     => esc_html__( 'Extra-Light 200', 'xoha-plus' ),
			'300'     => esc_html__( 'Light 300', 'xoha-plus' ),
			'400'     => esc_html__( 'Normal 400', 'xoha-plus' ),
			'500'     => esc_html__( 'Medium 500', 'xoha-plus' ),
			'600'     => esc_html__( 'Semi-Bold 600', 'xoha-plus' ),
			'700'     => esc_html__( 'Bold 700', 'xoha-plus' ),
			'800'     => esc_html__( 'Extra-Bold 800', 'xoha-plus' ),
			'900'     => esc_html__( 'Ultra-Bold 900', 'xoha-plus' ),
			'normal'  => esc_html__( 'Normal', 'xoha-plus' ),
			'normal'  => esc_html__( 'Normal', 'xoha-plus' ),
			'bold'    => esc_html__( 'Bold', 'xoha-plus' ),
			''        => esc_html__('Default', 'xoha-plus' ),
		);

		wp_localize_script( 'xoha-plus-typography-control', 'wdtPlusTypoObject', $typo_localize );
	}
	...
	public function get_custom_fonts() {
		$fonts = apply_filters( 'xoha_customizer_custom_fonts', array() );
		return $fonts;
	}
	...
	public function get_google_fonts() {
		...
		$google_fonts_file = XOHA_PLUS_DIR_PATH . 'customizer/controls/typography/google-fonts.json';
	...
	public function to_json() {
		...
		if ( ! is_array( $choices ) || empty( $choices ) ) {
			...
			$this->choices = array(
				'font_family'     => esc_html__( 'Font Family', 'xoha-plus'),
				'font_weight'     => esc_html__( 'Font Weight', 'xoha-plus'),
				'text_transform'  => esc_html__( 'Text Transform', 'xoha-plus'),
				'text_align'      => esc_html__( 'Text Align', 'xoha-plus'),
				'text_decoration' => esc_html__( 'Text Decoration', 'xoha-plus'),
				'font_style'      => esc_html__( 'Font Style', 'xoha-plus'),
				'font_size'       => esc_html__( 'Font Size', 'xoha-plus'),
				'line_height'     => esc_html__( 'Line Height', 'xoha-plus'),
				'letter_spacing'  => esc_html__( 'Letter Spacing', 'xoha-plus'),
			);
		}
		...
		<div class="wrapper">
			...
			<div class="full-width">
				...
				<select class="font-family">
					<option value=""><?php esc_html_e( 'Inherit', 'xoha-plus' );?></option>
					<optgroup label="<?php esc_html_e( 'Other System Fonts', 'xoha-plus' );?>">
					...
					<# if( _.isEqual( data.has_custom_fonts, {'item':'yes' } ) ) { #>
						<optgroup label="<?php esc_html_e( 'Custom Fonts', 'xoha-plus' );?>">
					...
					<optgroup label="<?php esc_html_e( 'Google', 'xoha-plus' );?>">
				...
				<# if( !_.isUndefined( data.choices['font_weight'] ) ) { #>
					<div class="one-half">
						...
						<select class="font-weight">
							<option value="inherit"><?php esc_html_e( 'Inherit', 'xoha-plus' );?></option>
						</select>
					</div>
				<# } #>
				...
				<# if( !_.isUndefined( data.choices['font_style'] ) ) { #>
					<div class="one-half">
						...
						<select class="font-style">
							...
							<option value="normal"<# if ( 'none' === data.value['font-style'] ) { #>selected<# } #>><?php esc_attr_e( 'Normal', 'xoha-plus' ); ?></option>
							<option value="italic"<# if ( 'italic' === data.value['font-style'] ) { #>selected<# } #>><?php esc_attr_e( 'Italic', 'xoha-plus' ); ?></option>
							<option value="oblique"<# if ( 'oblique' === data.value['font-style'] ) { #>selected<# } #>><?php esc_attr_e( 'Oblique', 'xoha-plus' ); ?></option>
							<option value="inherit"<# if ( 'inherit' === data.value['font-style'] ) { #>selected<# } #>><?php esc_attr_e( 'Inherit', 'xoha-plus' ); ?></option>
						</select>
					</div>
				<# } #>
			</div>
			...
			<# if( !_.isUndefined( data.choices['text_transform'] ) ) { #>
				<div class="full-width">
					...
					<select class="text-transform">
						...
						<option value="none"<# if ( 'none' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_attr_e( 'None', 'xoha-plus' ); ?></option>
						<option value="capitalize"<# if ( 'capitalize' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_attr_e( 'Capitalize', 'xoha-plus' ); ?></option>
						<option value="uppercase"<# if ( 'uppercase' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_attr_e( 'Uppercase', 'xoha-plus' ); ?></option>
						<option value="lowercase"<# if ( 'lowercase' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_attr_e( 'Lowercase', 'xoha-plus' ); ?></option>
						<option value="initial"<# if ( 'initial' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_attr_e( 'Initial', 'xoha-plus' ); ?></option>
						<option value="inherit"<# if ( 'inherit' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_attr_e( 'Inherit', 'xoha-plus' ); ?></option>
					</select>
				</div>
			<# } #>
				...
				<# if( !_.isUndefined( data.choices['text_align'] ) ) { #>
					<div class="one-half">
						...
						<select class="text-align">
							...
							<option value="unset"<# if ( 'unset' === data.value['text-align'] ) { #>selected<# } #>><?php esc_attr_e( 'Unset', 'xoha-plus' ); ?></option>
							<option value="left"<# if ( 'left' === data.value['text-align'] ) { #>selected<# } #>><?php esc_attr_e( 'Left', 'xoha-plus' ); ?></option>
							<option value="center"<# if ( 'center' === data.value['text-align'] ) { #>selected<# } #>><?php esc_attr_e( 'Center', 'xoha-plus' ); ?></option>
							<option value="right"<# if ( 'right' === data.value['text-align'] ) { #>selected<# } #>><?php esc_attr_e( 'Right', 'xoha-plus' ); ?></option>
							<option value="justify"<# if ( 'justify' === data.value['text-align'] ) { #>selected<# } #>><?php esc_attr_e( 'Justify', 'xoha-plus' ); ?></option>
							<option value="inherit"<# if ( 'inherit' === data.value['text-align'] ) { #>selected<# } #>><?php esc_attr_e( 'Inherit', 'xoha-plus' ); ?></option>
						</select>
					</div>
				<# } #>
				...
				<# if( !_.isUndefined( data.choices['text_decoration'] ) ) { #>
					<div class="one-half">
						...
						<select class="text-decoration">
							...
							<option value="none"<# if ( 'none' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_attr_e( 'None', 'xoha-plus' ); ?></option>
							<option value="underline"<# if ( 'underline' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_attr_e( 'Underline', 'xoha-plus' ); ?></option>
							<option value="overline"<# if ( 'overline' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_attr_e( 'Overline', 'xoha-plus' ); ?></option>
							<option value="line-through"<# if ( 'line-through' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_attr_e( 'Line Through', 'xoha-plus' ); ?></option>
							<option value="inherit"<# if ( 'inherit' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_attr_e( 'Inherit', 'xoha-plus' ); ?></option>
						</select>
					</div>
				<# } #>
			</div>
