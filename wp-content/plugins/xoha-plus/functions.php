...
if( !function_exists('xoha_plus_get_template_plugin_part') ) {
    function xoha_plus_get_template_plugin_part( $file_path, $module, $template, $slug ) {

        $html             = '';
        $template_path    = XOHA_PLUS_DIR_PATH . 'modules/' . esc_attr($module);
    ...
    add_filter( 'xoha_get_template_plugin_part', 'xoha_plus_get_template_plugin_part', 20, 4 );
}

if( !function_exists( 'xoha_customizer_panel_priority' ) ) {
    /**
     *  Get : Customizer Panel Priority based on panel name.
     */
    function xoha_customizer_panel_priority( $panel ) {
        $priority = 10;
        ...
        return apply_filters( 'xoha_customizer_panel_priority', $priority, $panel );
    }
}

if( !function_exists( 'xoha_customizer_settings' ) ) {
    /**
     * Get : Customizer settings value
     */
    function xoha_customizer_settings( $option ) {
        $settings = get_option( XOHA_CUSTOMISER_VAL, array() );
        $settings = isset( $settings[ $option ] ) ? $settings[ $option ] : false;
        return $settings;
    }
}

if( !function_exists( 'xoha_customizer_dynamic_style') ) {
    /**
     * Get : Generate style based on selector and property
     */
    function xoha_customizer_dynamic_style( $selectors, $properties ) {
        $output = '';
        if( !empty( $selectors ) && !empty( $properties ) ) {
            ...
            $output .= ' { ' . xoha_html_output($properties) . ' } ' . "\n";
        }
        return $output;
    }
}
...
if( !function_exists( 'xoha_customizer_responsive_typography_settings' ) ) {

    /**
     * Get : Typography Responsive CSS based on option and responsive mode.
     */
    function xoha_customizer_responsive_typography_settings( $option, $mode = 'tablet' ) {
        $css = '';
...
if( !function_exists( 'xoha_customizer_typography_settings' ) ) {
    /**
     * Get : Typography CSS based on option.
     */
    function xoha_customizer_typography_settings( $option ) {
...
if( !function_exists( 'xoha_customizer_frontend_font' ) ) {
    /**
     * Load fonts in frontend
     */
    function xoha_customizer_frontend_font( $settings, $fonts ) {
        $font = '';
...
if( !function_exists( 'xoha_customizer_color_settings' ) ) {
    function xoha_customizer_color_settings( $color ) {
        $css = '';
...
if( !function_exists( 'xoha_customizer_bg_color_settings' ) ) {
    function xoha_customizer_bg_color_settings( $color ) {
        $css = '';
...
if( !function_exists( 'xoha_customizer_bg_settings' ) ) {
    function xoha_customizer_bg_settings( $bg ) {
        $css = '';
...
# Field Sanitization
if(!function_exists('xoha_sanitization')) {
	function xoha_sanitization($data) {
		if ( is_array( $data ) && !empty( $data ) ) {
			foreach ( $data as $key => &$value ) {
				if ( is_array( $value ) ) {
					$data[$key] = xoha_sanitization($value);
				} else {
