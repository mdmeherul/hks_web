...
if( !class_exists( 'Xoha_Pro_Woo_Sidebar' ) ) {
    class Xoha_Pro_Woo_Sidebar {
        ...
        function frontend() {
            add_filter('xoha_primary_classes', array( $this, 'primary_classes' ) );
            add_filter('xoha_secondary_classes', array( $this, 'secondary_classes' ) );
            add_filter('xoha_active_sidebars', array( $this, 'active_sidebars' ), 20 );
            add_filter('xoha_woo_loop_column_class', array( $this, 'woo_loop_column_class' ), 10, 2 );
        }

        function primary_classes( $primary_class ) {

            if( is_shop()  ) {
                $settings = get_post_meta( get_option('woocommerce_shop_page_id'), '_xoha_layout_settings', true );
