...
if( !class_exists( 'XohaProBCTemplate' ) ) {
    class XohaProBCTemplate {
        ...
        function fonts_list( $fonts ) {
            $title = xoha_customizer_frontend_font( xoha_customizer_settings('breadcrumb_title_typo'), array() );
            if( count( $title ) ) {
                array_push( $fonts, $title[0] );
            }

            $content = xoha_customizer_frontend_font( xoha_customizer_settings('breadcrumb_typo'), array() );
            if( count( $content ) ) {
        ...
        function base_style( $style ) {
            ...
            $bg                = xoha_customizer_settings('breadcrumb_background');
            $enable_overlay    = xoha_customizer_settings('breadcrumb_overlay_bg_color');
            $gradient_color    = xoha_customizer_settings('breadcrumb_background_gradient_color');
            $title_typo        = xoha_customizer_settings('breadcrumb_title_typo');
            $title_color       = xoha_customizer_settings('breadcrumb_title_color');
            $content_typo      = xoha_customizer_settings('breadcrumb_typo');
            $content_color     = xoha_customizer_settings('breadcrumb_text_color');
            $content_a_color   = xoha_customizer_settings('breadcrumb_link_color');
            $content_a_h_color = xoha_customizer_settings('breadcrumb_link_hover_color');
            ...
            if( !empty( $title_css ) ) {
                $style .= xoha_customizer_dynamic_style( '.dark-bg-breadcrumb .main-title-section h1,  .main-title-section h1', $title_css );
            }
            ...
            if( !empty( $content_css ) ) {
                $style .= xoha_customizer_dynamic_style( '.dark-bg-breadcrumb .breadcrumb, .dark-bg-breadcrumb .breadcrumb span.current, .breadcrumb, .breadcrumb span.current', $content_css );
            }

            $content_a_css = xoha_customizer_color_settings( $content_a_color );
            if( !empty( $content_a_css ) ) {
                $style .= xoha_customizer_dynamic_style( '.dark-bg-breadcrumb .breadcrumb a, .breadcrumb a', $content_a_css );
            }

            $content_a_h_css = xoha_customizer_color_settings( $content_a_h_color );
            if( !empty( $content_a_h_css ) ) {
                $style .= xoha_customizer_dynamic_style( '.dark-bg-breadcrumb .breadcrumb a:hover, .breadcrumb a:hover', $content_a_h_css );
            }

            if(is_singular()) {
                global $post;
                $post_meta = get_post_meta( $post->ID, '_xoha_breadcrumb_settings', true );
        ...
        function tablet_landscape_style( $style ) {
            ...
            if( !empty( $title_typo_css) ) {
                $style .= xoha_customizer_dynamic_style( '.main-title-section h1', $title_typo_css );
            }

            $content_typo     = xoha_customizer_settings('breadcrumb_typo');
            $content_typo_css = xoha_customizer_responsive_typography_settings( $content_typo, 'tablet-ls' );
            if( !empty( $content_typo_css) ) {
                $style .= xoha_customizer_dynamic_style( '.breadcrumb', $content_typo_css );
            }
