<?php
class Xoha_Widget_Recent_Posts extends WP_Widget {
	#1.constructor
	function __construct() {
		$widget_options = array(
			'classname'   => 'widget_recent_posts',
			'description' => esc_html__('To list out posts', 'xoha-pro')
		);

        $theme_name =  defined('XOHA_THEME_NAME') ? XOHA_THEME_NAME : 'Xoha';
		parent::__construct( false, $theme_name . esc_html__(' Posts','xoha-pro'), $widget_options );
	}
	...
	function form($instance) {
	    ...
	    <p>
	    	<label for="<?php echo esc_attr($this->get_field_id('_post_categories')); ?>">
	    		<?php esc_html_e('Choose the categories you want to display (multiple selection possible)','xoha-pro');?>
	    	</label>
            <select class="widefat" id="<?php echo esc_attr($this->get_field_id('_post_categories').'[]');?>" name="<?php echo esc_attr($this->get_field_name('_post_categories').'[]');?>" multiple="multiple">
            	<option value=""><?php esc_html_e("Select",'xoha-pro');?></option><?php
        ...
        <p>
        	<label for="<?php echo esc_attr($this->get_field_id('_excerpt')); ?>">
        		<?php esc_html_e('Display title only or title &amp; excerpt','xoha-pro');?>
        	</label>
        ...
        <p>
        	...
        	<?php esc_html_e("Show Image",'xoha-pro');?>
        </p>

	    <p>
	    	<label for="<?php echo esc_attr($this->get_field_id('_post_count'));?>">
	    		<?php esc_html_e('No.of posts to show:','xoha-pro');?>
	    	</label>
	...
	function widget($args, $instance) {
		...
		echo "<div class='recent-posts-widget'><ul>";
			 ...
			 while( $the_query->have_posts() ):
				...
				echo "<li>";
					if(1 == $_enabled_image):
						...
						$image = ( $image != false)? $image[0] : XOHA_PRO_DIR_URL . 'modules/blog/assets/images/post-thumb.jpg';
					...
					if($show_title):
						...
						echo "<div class='entry-title'><h4><a href='".get_permalink()."'>{$title}</a></h4></div>";
						echo "<div class='entry-content'>".xoha_excerpt(10)."</div>";
					endif;

					echo '<div class="entry-meta">';
						echo '<p> <span class="wdticon-clock-o"> </span> '.get_the_date('M').' '.get_the_date('d').'</p>';
						$commtext = "";
			 ...
			 endwhile;
			 	...
			 	echo "<li><h4>".esc_html__('No Posts found','xoha-pro')."</h4></li>";
			 endif;
			 wp_reset_postdata();
	 	echo "</ul></div>";
