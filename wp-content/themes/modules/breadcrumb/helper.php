<?php
add_action( 'xoha_after_main_css', 'breadcrumb_style' );
function breadcrumb_style() {
    wp_enqueue_style( 'xoha-breadcrumb', get_theme_file_uri('/modules/breadcrumb/assets/css/breadcrumb.css'), false, XOHA_THEME_VERSION, 'all');
}

if( ! function_exists( 'xoha_breadcrumb_template' )  ) {
	function xoha_breadcrumb_template() {
        if ( !is_front_page() && !is_home() ) {
		    xoha_template_part( 'breadcrumb', 'templates/title' );
        }
	}

	add_action( 'xoha_breadcrumb', 'xoha_breadcrumb_template' );
}
function xoha_breadcrumb_params() {
    $params = array(
        "home"            => esc_html__('Home','xoha'),
        "home_link"       => home_url('/'),
        "delimiter"       => '<span class="breadcrumb-default-delimiter"></span>',
        "wrapper_classes" => "default overlay-wrapper"
    );
    return apply_filters( 'xoha_breadcrumb_params', $params );
}

function xoha_breadcrumb_title() {
    $title = get_the_title( xoha_get_page_id() );
	...
	if ( ( is_home() && is_front_page() ) || is_singular('attachment') ) {
        ...
        $title = esc_html__("Search Result for: ",'xoha').' '.get_search_query();
    }

    return apply_filters( 'xoha_breadcrumb_title', '<h1>'.$title.'</h1>' );
}

function xoha_breadcrumbs( $home, $separator ) {
    $output      = '';
    $breadcrumbs = array();
    ...
    if( is_search() ) {
    	$breadcrumbs[] = '<a href="javascript:void(0);">' . esc_html__('Search', 'xoha') . '</a>';
    }
    ...
    $filtered_breadcrumbs = apply_filters( 'xoha_breadcrumbs', $breadcrumbs );
