<?php
add_action( 'xoha_after_main_css', 'notfound_style' );
function notfound_style() {
    if( is_404() ) {
        wp_enqueue_style( 'xoha-404', get_theme_file_uri('/modules/404/assets/css/404.css'), false, XOHA_THEME_VERSION, 'all');
    }
}

add_filter( 'xoha_add_inline_style', 'notfound_inline_style' );
function notfound_inline_style( $style ) {
    if( is_404() ) {
        $params = xoha_404_page_params();
...
function xoha_404_page_params() {
    ...
    return apply_filters( 'xoha_404_page_params', $params );
}